...
class BorrowLendRatesWrapper:
    def __init__(self, env: AppEnvironment):
        ...
        with urllib.request.urlopen(env["api_url"]) as res:
    ...
    def lend_rates(self) -> dict[str, float]:
        ...
        for item in self._data.borrow_lend:
            if item.symbol in self._env["lend_targets"]:
                symbol_to_rate[item.symbol] = item.lend_rate

        if not symbol_to_rate:
            raise ValueError(
                f"指定したシンボル {self._env['lend_targets']} と一致する貸出レートが取得できませんでした。"
            )
        return symbol_to_rate

    def borrow_rates(self) -> dict[str, float]:
        """シンボルから 貸出金利 を索引する dictを返す 例: {"USDC": 0.02, "USDT": 0.03}"""
        ...
        symbol_to_rate: dict[str, float] = {}
        for item in self._data.borrow_lend:
            if item.symbol in self._env["borrow_targets"]:
                symbol_to_rate[item.symbol] = item.borrow_rate

        if not symbol_to_rate:
            raise ValueError(
                f"指定したシンボル {self._env['borrow_targets']} と一致する借入レートが取得できませんでした。"
            )
        return symbol_to_rate
